...
class Bert(pl.LightningModule):
    ...
    def __init__(
        ...
        self.tag_pred = nn.Linear(self.embedding_dim, self.label_size)
        # 输出模型对 标记输出的概率 （未归一化）

        self.crf = CRF(self.label_size, batch_first=True)

    def forward(self, word, label, mask):
        representation = self.bert(word, attention_mask=mask)[0]
        bsz, seqlen, _ = representation.size()
        tag_preds = self.tag_pred(
            representation.view(bsz * seqlen, self.embedding_dim)).view(
                bsz, seqlen, self.label_size)
